...
$LogFile = "$($PSScriptRoot)\logs\$($CloneName)_" + ((Get-Date).ToString('ddMMyyHHmmss')) + '_CloneVM.log'
# rename and move
Copy-Item -Path 
Rename-Item -Path "$($PSScriptRoot)\logs\*.log" -NewName "$($CloneName)_OldLog_" + ((Get-Date).ToString('ddMMyyHHmmss')) + '.log' -Force
